...
const pokemon = [
    ...
    { name: "Pidgey", type: "Normal", HP: 40 },
    { name: "Rattata", type: "Normal", HP: 30 },
  ...
  const additionalPokemons = [
    ...
    pokemon.find(p => p.HP > 40) 
  ];
  

  additionalPokemons.forEach(pokemon => {
    if (pokemon) {
...
*/
let starterPokemon = game.party.find(p => p.starter === true);


let evolvedPokemon;

if (starterPokemon) {
  switch (starterPokemon.name) {
    case "Bulbasaur":
      evolvedPokemon = { name: "Ivysaur", type: "Grass/Poison", HP: 60, starter: false };
      break;
    case "Charmander":
      evolvedPokemon = { name: "Charmeleon", type: "Fire", HP: 58, starter: false };
      break;
    case "Squirtle":
      evolvedPokemon = { name: "Wartortle", type: "Water", HP: 59, starter: false };
      break;
    case "Pikachu":
      evolvedPokemon = { name: "Raichu", type: "Electric", HP: 90, starter: false };
      break;
    default:
...
game.catchPokemon = function(pokemonObj) {
    const pokeballItem = this.items.find(item => item.name === "pokeball"); // العثور على الكرات
    if (pokeballItem) {
  ...
  const chosenPokemon = pokemon.find(p => p.name === "Rattata");
  game.catchPokemon(chosenPokemon);
  console.log(game.party);
